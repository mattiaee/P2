...
TableView::TableView(Model*& m, QWidget* parent) : QWidget(parent), model(m) {
    ...
    table = new QTableWidget(model->rowCount()+1, model->columnCount()+1);
    //gestisco visualizzazione label
    for(int i=0;i<model->columnCount();i++) addColHeaderItem(i,model->getColLabel(i));
    for(int j=0;j<model->rowCount();j++) addRowHeaderItem(j,model->getRowLabel(j));
...
void TableView::insertColumn() {
    if(model && table) {
        ...
        model->insertColumn();
        addColHeaderItem(model->columnCount()-1, model->getColLabel(model->columnCount()-1));
...
void TableView::insertRow() {
    if(model && table) {
        ...
        model->insertRow();
        addRowHeaderItem(model->rowCount()-1, model->getRowLabel(model->rowCount()-1));
...
void TableView::changeColText(int col, std::string t) {model->setColLabel(col,t);};
